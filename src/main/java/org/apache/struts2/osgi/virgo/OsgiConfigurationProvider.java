...
public class OsgiConfigurationProvider implements PackageProvider, BundleListener {
    ...
    public void init(Configuration configuration) throws ConfigurationException {
        osgiHost = (OsgiHost) servletContext.getAttribute(StrutsOsgiVirgoListener.OSGI_HOST);
    ...
    protected boolean shouldProcessBundle(Bundle bundle) {
    	return OsgiUtil.hasStrutsHostHeader(bundle);
    }
...
//    }
    ...
    protected void onBundleStopped(Bundle bundle) {
        ...
        if (!packages.isEmpty()) {
            if (LOG.isTraceEnabled())
                LOG.trace("The bundle [#0] has been stopped. The packages [#1] will be disabled", bundle.getSymbolicName(), packages.toString());
